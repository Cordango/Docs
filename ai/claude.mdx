...
When there's no app yet, describe the job rather than the data model. Cordango's whole point is
working out the second from the first.
...
<Tip>
"Stop and show me the plan first" is worth the extra turn. The one change that's genuinely painful
to undo later is duplicating an entity a core app already provides, and that decision gets made in
