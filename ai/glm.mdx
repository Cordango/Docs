...
The long timeout isn't decoration. A whole-app generation pass is a long single call, and the
...
check` is free, so there's no reason for a step to be reported rather than run.

**Watch for a second copy of a core entity.** The Phase 1 stop exists for this. A `company` or
`customer` entity appearing in Phase 2 means Phase 1 got skimmed, and that's the one mistake that's
...
revert it and re-ask with the sequence written out. That's cheaper than reviewing it.
