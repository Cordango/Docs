...
What it produces serves three faces from one process — the application, a REST API documented at
`/scalar` from a real OpenAPI 3.1 document at `/openapi/v1.json`, and an [MCP endpoint](/mcp/overview)
at `/mcp` — all three going through the same permissions.
